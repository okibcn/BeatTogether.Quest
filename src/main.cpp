...
{
public:
    ModConfig() : hostname(HOST_NAME), port(PORT), statusUrl(STATUS_URL), button("Beat\nTogether") {}
    ...
    {
        getLogger().info("Server address: %s:%i", to_utf8(csstrtostr(hostnameStr)).c_str(), port);
...
extern "C" void load()
{
    // Loads custom config from /sdcard/ModData/com.beatgames.beatsaber/Configs/BeatTogether.cfg
